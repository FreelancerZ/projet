<?php
	...
	function debannir($idUser) {
		...
		if ($data['user_admin'] == 1) {
			...
			// bannisement de l'utilisateur
			$sql = "UPDATE users SET user_banni = 0 WHERE user_id = :id";
			$req = $bdd->prepare($sql);
			$req->bindParam(':id', $idUser);
			...
			echo "<script>window.alert('Cet utilisateur n'est désormais plus banni');</script>";

			// envoi du mail
			$sujet = "[Freelancerz] : Votre compte n'est plus suspendu !";
			$entete = "From: noreply@freelancerZ.com";

			$contenu = 'Bonjour,

		Votre compte a été débloqué. Vous pouvez de nouveau accéder à nos services.
